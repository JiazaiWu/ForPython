...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    int_source = [[source_vocab_to_int.get(word) for word in sentence.split()]
                  for sentence in source_text.split('\n')]
    int_target = [[target_vocab_to_int.get(word) for word in sentence.split()] + [int_eos]
                  for sentence in target_text.split('\n')]
    return int_source, int_target
...
def process_decoding_input(target_data, target_vocab_to_int, batch_size):
    ...
    # tf.fill([batch_size, 1]: vector which value is int_go and shape is [batch_size x 1]
    # tf.concat([m1,m2], 1): link the rows
    dec_input = tf.concat([tf.fill([batch_size, 1], int_go), ending], 1)
    return dec_input
...
def encoding_layer(rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    dropped_cell = tf.contrib.rnn.DropoutWrapper(lstm_cell, output_keep_prob=keep_prob)
    mul_cel = tf.contrib.rnn.MultiRNNCell([dropped_cell] * num_layers)
    _, output_state = tf.nn.dynamic_rnn(mul_cel, rnn_inputs, dtype=tf.float32)
    return output_state
...
def decoding_layer(dec_embed_input, dec_embeddings, encoder_state, vocab_size, sequence_length, rnn_size,
    ...
    """
    # TODO: Implement Function
    int_start = target_vocab_to_int['<GO>']
    int_end = target_vocab_to_int['<EOS>']
    # Decoder RNNs
    ...
    with tf.variable_scope("decoding") as decoding_scope:
        # Output Layer
        tarin_logits = decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
                         output_fn, keep_prob)
    with tf.variable_scope("decoding", reuse=True) as decoding_scope:
        inference_logits = decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, int_start, int_end,
                                                sequence_length - 1, vocab_size, decoding_scope, output_fn,
                                                keep_prob)
    return tarin_logits, inference_logits
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    enc_output_state = encoding_layer(enc_embed_input, rnn_size, num_layers, keep_prob)
    
    dec_input = process_decoding_input(target_data, target_vocab_to_int, batch_size)
    dec_embeddings = tf.Variable(tf.random_uniform([target_vocab_size, dec_embedding_size]))
    dec_embed_input = tf.nn.embedding_lookup(dec_embeddings, dec_input)

    train_logits, inference_logits = decoding_layer(dec_embed_input, dec_embeddings, enc_output_state, target_vocab_size,
...
DON'T MODIFY ANYTHING IN THIS CELL
"""
# change save_patch to './save' to make pass
# save = 'checkpoint/dev'
save_path = './save'
(source_int_text, target_int_text), (source_vocab_to_int, target_vocab_to_int), _ = helper.load_preprocess()
...
def sentence_to_seq(sentence, vocab_to_int):
    ...
    """
    # TODO: Implement Function
    int_unk = vocab_to_int['<UNK>']
    return [vocab_to_int.get(word, int_unk) for word in sentence.lower().split()]
